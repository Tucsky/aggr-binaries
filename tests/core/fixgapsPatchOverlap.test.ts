...
test("fixgaps still patches binaries when recovered trades are deduped (inserted=0)", async () => {
  const fixture = await createFixture();
  const db = openDatabase(fixture.dbPath);

  try {
    const fileAPath = path.join(fixture.root, fixture.relativePathA);
    await fs.writeFile(
      fileAPath,
      [
        `${TRADE_A0} 150 1 1 0`,
        `${TRADE_MISSING} 100 1 1 0`,
        `${TRADE_A1} 200 1 1 0`,
        `${TRADE_A2} 201 1 0 0`,
      ].join("\n"),
    );
    ...
    const missingSlot = Math.floor(TRADE_MISSING / 60_000) * 60_000;
    await writeCandleAtSlot(fixture.outDir, "1m", missingSlot, zeroCandle());
    await writeCandleAtSlot(fixture.outDir, "4h", SLOT_4H, zeroCandle());

    assert.strictEqual((await readCandleAtSlot(fixture.outDir, "1m", missingSlot)).open, 0);
    assert.strictEqual((await readCandleAtSlot(fixture.outDir, "4h", SLOT_4H)).open, 0);

    db.db.exec("DELETE FROM events;");
    insertGapEvent(db, {
      rootId,
      relativePath: fixture.relativePathA,
      gapMs: TRADE_A1 - TRADE_A0,
      gapEndTs: TRADE_A1,
    });
    ...
    const stats = await runFixGaps(buildConfig(fixture.root, fixture.outDir, fixture.dbPath, "1m"), db, {
      adapterRegistry: createAdapterRegistry({
        KRAKEN: {
          name: "kraken-test",
          async recover() {
            // Duplicate row: merge inserts 0, but patching should still run on recovered bounds.
            return [{ ts: TRADE_MISSING, price: 100, size: 1, side: "buy", priceText: "100", sizeText: "1" }];
          },
        },
      }),
    ...
    const after1m = await readCandleAtSlot(fixture.outDir, "1m", missingSlot);
    assert.strictEqual(after1m.open, 1_000_000);
    assert.strictEqual(after1m.close, 1_000_000);
    assert.strictEqual(after1m.buyCount, 1);
